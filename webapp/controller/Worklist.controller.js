	...
	return BaseController.extend("mycompany.myapp.controller.Worklist", {
		...
		_showObject: function(oItem) {
			this.getRouter().navTo("object", {
				objectId: oItem.getBindingContext("groups").getProperty("ProductID")
			});
		},
		...
		onUnlistObjects: function() {
			...
			if (aSelectedProducts.length) {
				for (i = 0; i < aSelectedProducts.length; i++) {
					...
					oProductId = oProduct.getBindingContext("groups").getProperty("ProductID");
					sPath = oProduct.getBindingContext("groups").getPath();
		...
		onUpdateStockObjects: function() {
			...
			if (aSelectedProducts.length) {
				for (i = 0; i < aSelectedProducts.length; i++) {
					sPath = aSelectedProducts[i].getBindingContext("groups").getPath();
					oProductObject = aSelectedProducts[i].getBindingContext("groups").getObject();
